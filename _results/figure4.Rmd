...
## Create multiple realizations of Fig 4d (total housing units)
...
samp1 <- sample(1:nrow(df.pca.cty.1981), nsamp)
x1 <- xbase + 
  geom_point(
    data = transfm(df.pca.cty.1981, xname) %>% slice(samp1) %>% 
      mutate(y = shap.cty.1981 %>% data.frame %>% slice(samp1) %>% pull(xname)),
    aes(x = x, y = y), size = ptsize, color = ptcol, alpha = 0.25) + 
  theme(
    axis.title.y = element_text(),
    axis.text.y = element_text())

samp2 <- sample(1:nrow(df.pca.cty.1981), nsamp)
x2 <- xbase + 
  geom_point(
    data = transfm(df.pca.cty.1981, xname) %>% slice(samp2) %>% 
      mutate(y = shap.cty.1981 %>% data.frame %>% slice(samp2) %>% pull(xname)),
    aes(x = x, y = y), size = ptsize, color = ptcol, alpha = 0.25)

samp3 <- sample(1:nrow(df.pca.cty.1981), nsamp)
x3 <- xbase + 
  geom_point(
    data = transfm(df.pca.cty.1981, xname) %>% slice(samp3) %>% 
      mutate(y = shap.cty.1981 %>% data.frame %>% slice(samp3) %>% pull(xname)),
    aes(x = x, y = y), size = ptsize, color = ptcol, alpha = 0.25)

samp4 <- sample(1:nrow(df.pca.cty.1981), nsamp)
x4 <- xbase + 
  geom_point(
    data = transfm(df.pca.cty.1981, xname) %>% slice(samp4) %>% 
      mutate(y = shap.cty.1981 %>% data.frame %>% slice(samp4) %>% pull(xname)),
    aes(x = x, y = y), size = ptsize, color = ptcol, alpha = 0.25)

x1 + x2 + x3 + x4 + plot_layout(nrow = 1)
ggsave('_figures/review_4d.png', width = 8, height = 2.25)

```

## Create multiple realizations of Fig 4e (percent population in floodplain)
...
x1 + x2 + x3 + x4 + plot_layout(nrow = 1)
ggsave('_figures/review_4e.png', width = 8, height = 2.25)
