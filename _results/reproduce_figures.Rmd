...
suppressPackageStartupMessages({
  ...
  require(iml)
  require(ggrepel)
  require(extrafont); windowsFonts("Segoe UI" = windowsFont("Segoe UI"))
})
...
```{r}
g1 + g2 + g3 + guide_area() + 
  plot_layout(design = 'abc\nddd', heights = c(10,1), guides = 'collect') +
  plot_annotation(tag_levels = 'a', tag_prefix = '(', tag_suffix = ')') &
  theme(
    plot.tag = element_text(face = 'bold'),
    plot.tag.position = c(0.12,0.975)) #1.01 without the title
ggsave('_figures/fig5_interaction.png', width = 6, height = 2.5)
...
```{r}
g1 + g2 + g3 + g4 + g5 + g6 + guide_area() + 
  plot_layout(design = 'abc\ndef\nggg', guides = 'collect', heights = c(10,10,1)) + 
  plot_annotation(tag_levels = 'a', tag_prefix = '(', tag_suffix = ')') &
  theme(
    plot.tag = element_text(face = 'bold'),
    plot.tag.position = c(0.12,0.975)) #1.01 without the title
ggsave('_figures/review_5.png', width = 6, height = 4.5)

```

# Respond to reviewer comments

add pop in floodplain & median income to fig 5
