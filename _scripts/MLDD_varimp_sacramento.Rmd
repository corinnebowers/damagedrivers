...
load(paste0('_results/sac_rfmodel_',i,'.Rdata'))
load(paste0('_results/sac_pdp_',i,'.Rdata'))
load(paste0('_results/sac_iml_',i,'.Rdata'))
load(paste0('_results/sac_shap_',i,'.Rdata'))
...
partials.ice[[1]] %>% 
  ...
  ggplot() + 
  geom_line(
    aes(x = 10^logprecip_total-1, y = c.ice, group = yhat.id, color = y),
    size = 0.01, alpha = 0.25, #color = 'grey50', 
    stat = 'smooth', method = 'loess', span = 0.5,
    show.legend = FALSE) + 
  geom_line(
    data = partials[[1]],
    aes(x = 10^logprecip_total-1, y = yhat-yhat[1]),
    size = 1) + 
  geom_rug(
    data = df.pca %>% slice(samp), aes(x = 10^logprecip_total-1)) + 
  labs(x = importance.plot$fullname[1])
...
#   ggplot() +
#   geom_line(
#     aes(x = 10^logprecip_total-1, y = yhat, group = yhat.id, color = y),
#     size = 0.01, alpha = 0.25, #color = 'grey50',
#     stat = 'smooth', method = 'loess', span = 0.5) +
#   geom_line(
#     data = partials[[1]],
#     aes(x = 10^logprecip_total-1, y = yhat),
#     size = 1) +
#   geom_point(
#     data = df.pca %>% slice(samp),
#     aes(
#       x = 10^logprecip_total-1,
#       y = predict(f.rf, df.pca %>% slice(samp), type = 'prob')$yes),
#     size = 1, color = 'grey40') +
#   scale_y_origin() + coord_cartesian(ylim = c(0,1))
...
partials.ice[[2]] %>% 
  ...
  ggplot() + 
  geom_line(
    aes(x = maxivt, y = c.ice, group = yhat.id, color = y),
    size = 0.01, alpha = 0.25, #color = 'grey50', 
    stat = 'smooth', method = 'loess', span = 0.5,
    show.legend = FALSE) + 
  geom_line(
    data = partials[[2]],
    aes(x = maxivt, y = yhat-yhat[1]),
    size = 1) + 
  geom_rug(
    data = df.pca %>% slice(samp), aes(x = maxivt)) + 
  labs(x = importance.plot$fullname[2])
...
#   ggplot() + 
#   geom_line(
#     aes(x = maxivt, y = yhat, group = yhat.id, color = y),
#     size = 0.01, alpha = 0.25, #color = 'grey50', 
#     stat = 'smooth', method = 'loess', span = 0.5) + 
#   geom_line(
#     data = partials[[2]],
...
partials.ice[[3]] %>% 
  ...
  ggplot() + 
  geom_line(
    aes(x = 10^logprecip_max-1, y = c.ice, group = yhat.id, color = y),
    size = 0.01, alpha = 0.25, #color = 'grey50', 
    stat = 'smooth', method = 'loess', span = 0.5,
    show.legend = FALSE) + 
  geom_line(
    data = partials[[3]],
    aes(x = 10^logprecip_max-1, y = yhat-yhat[1]),
    size = 1) + 
  geom_rug(
    data = df.pca %>% slice(samp), aes(x = 10^logprecip_max-1)) + 
  labs(x = importance.plot$fullname[3])

partials.ice[[4]] %>% #names
  left_join(obs.id %>% cbind(y = df.pca$y), by = c('yhat.id' = 'i')) %>% 
  filter(yhat.id %in% samp) %>% 
  group_by(yhat.id) %>% mutate(c.ice = yhat-yhat[1]) %>% ungroup %>% 
  ggplot() + 
  geom_line(
    aes(x = duration, y = c.ice, group = yhat.id, color = y),
    size = 0.01, alpha = 0.25, #color = 'grey50', 
    stat = 'smooth', method = 'loess', span = 0.5,
    show.legend = FALSE) + 
  geom_line(
    data = partials[[4]],
    aes(x = duration, y = yhat-yhat[1]),
    size = 1) + 
  geom_rug(
    data = df.pca %>% slice(samp), aes(x = duration)) + 
  labs(x = importance.plot$fullname[4])

partials.ice[[5]] %>% #names
  left_join(obs.id %>% cbind(y = df.pca$y), by = c('yhat.id' = 'i')) %>% 
  filter(yhat.id %in% samp) %>% 
  group_by(yhat.id) %>% mutate(c.ice = yhat-yhat[1]) %>% ungroup %>% 
  ggplot() + 
  geom_line(
    aes(x = 10^logprecip_lag07-1, y = c.ice, group = yhat.id, color = y),
    size = 0.01, alpha = 0.25, #color = 'grey50', 
    stat = 'smooth', method = 'loess', span = 0.5,
    show.legend = FALSE) + 
  geom_line(
    data = partials[[5]],
    aes(x = 10^logprecip_lag07-1, y = yhat-yhat[1]),
    size = 1) + 
  geom_rug(
    data = df.pca %>% slice(samp), aes(x = 10^logprecip_lag07-1)) + 
  labs(x = importance.plot$fullname[5])

partials.ice[[6]] %>% #names
  left_join(obs.id %>% cbind(y = df.pca$y), by = c('yhat.id' = 'i')) %>% 
  filter(yhat.id %in% samp) %>% 
  group_by(yhat.id) %>% mutate(c.ice = yhat-yhat[1]) %>% ungroup %>% 
  ggplot() + 
  geom_line(
    aes(x = med_struct_age, y = c.ice, group = yhat.id, color = y),
    size = 0.01, alpha = 0.25, #color = 'grey50', 
    stat = 'smooth', method = 'loess', span = 0.5,
    show.legend = FALSE) + 
  geom_line(
    data = partials[[6]],
    aes(x = med_struct_age, y = yhat-yhat[1]),
    size = 1) + 
  geom_rug(
    data = df.pca %>% slice(samp), aes(x = med_struct_age)) + 
  labs(x = importance.plot$fullname[6])
...
#   ggplot() + 
#   geom_line(
#     aes(x = med_struct_age, y = yhat, group = yhat.id, color = y),
#     size = 0.01, alpha = 0.25, #color = 'grey50', 
#     stat = 'smooth', method = 'loess', span = 0.5) + 
#   geom_line(
#     data = partials[[6]],
#     aes(x = med_struct_age, y = yhat),
#     size = 1) +
#   geom_point(
#     data = df.pca %>% slice(samp),
#     aes(
...
```{r}
shap %>% as.data.frame %>% select(-y) %>% slice(samp) %>%
  ggplot() +
  geom_hline(yintercept = 0, color = 'grey50') + 
  geom_point(
    aes(
      y = get(importance.plot$varnames[2]), 
      x = df.pca %>% slice(samp) %>% pull(importance.plot$varnames[2])), 
    alpha = 0.25) +
  geom_line(
    data = iml.ale %>% filter(.feature == importance.plot$varnames[2]),
    aes(x = .borders, y = .value), color = 'red', size = 1) + 
  theme(panel.grid.major.y = element_line(size = 0.25)) + 
  labs(x = importance.plot$fullname[2], y = 'Effect (??)')
...
shap %>% as.data.frame %>% select(-y) %>% slice(samp) %>%
  ...
  geom_point(
    aes(
      y = get(importance.plot$varnames[3]), 
      x = 10^(df.pca %>% slice(samp) %>% pull(importance.plot$varnames[3]))-1), 
    alpha = 0.25) +
  geom_line(
    data = iml.ale %>% filter(.feature == importance.plot$varnames[3]),
    aes(x = 10^.borders-1, y = .value), color = 'red', size = 1) + 
  theme(panel.grid.major.y = element_line(size = 0.25)) + 
  labs(x = importance.plot$fullname[3], y = 'Effect (??)')
...
shap %>% as.data.frame %>% select(-y) %>% slice(samp) %>%
  ...
  geom_point(
    aes(
      y = get(importance.plot$varnames[4]), 
      x = df.pca %>% slice(samp) %>% pull(importance.plot$varnames[4])), 
    alpha = 0.25) +
  geom_line(
    data = iml.ale %>% filter(.feature == importance.plot$varnames[4]),
    aes(x = .borders, y = .value), color = 'red', size = 1) + 
  theme(panel.grid.major.y = element_line(size = 0.25)) + 
  labs(x = importance.plot$fullname[4], y = 'Effect (??)')
...
shap %>% as.data.frame %>% select(-y) %>% slice(samp) %>%
  ...
  geom_point(
    aes(
      y = get(importance.plot$varnames[5]), 
      x = 10^(df.pca %>% slice(samp) %>% pull(importance.plot$varnames[5]))-1), 
    alpha = 0.25) +
  geom_line(
    data = iml.ale %>% filter(.feature == importance.plot$varnames[5]),
    aes(x = 10^.borders-1, y = .value), color = 'red', size = 1) + 
  theme(panel.grid.major.y = element_line(size = 0.25)) + 
  labs(x = importance.plot$fullname[5], y = 'Effect (??)')
...
shap %>% as.data.frame %>% select(-y) %>% slice(samp) %>%
  ...
  geom_point(
    aes(
      y = get(importance.plot$varnames[6]), 
      x = df.pca %>% slice(samp) %>% pull(importance.plot$varnames[6])), 
    alpha = 0.25) +
  geom_line(
    data = iml.ale %>% filter(.feature == importance.plot$varnames[6]),
    aes(x = .borders, y = .value), color = 'red', size = 1) + 
  theme(panel.grid.major.y = element_line(size = 0.25)) + 
  labs(x = importance.plot$fullname[6], y = 'Effect (??)')
