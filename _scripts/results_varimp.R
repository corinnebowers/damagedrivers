...
# if (geom == 'county') {
#   cat('calculating Shapley values by county...\n')
#   pb <- txtProgressBar(min = 0, max = length(unique(obs.id$id)), style = 3)
#   timer <- Sys.time()
#   cl <- makeCluster(cores)
#   registerDoSNOW(cl)
#   shap.county <-
#     foreach (
#       i = sort(unique(obs.id$id)),
#       .packages = c('iml', 'randomForest', 'tidyverse'),
#       .combine = 'rbind',
#       .options.snow = opts) %dopar% {
#         Shapley$new(predictor, x.interest = df.pca[obs.id$id==i,])$results %>%
#           data.frame %>% column_to_rownames('feature') %>% select(phi) %>% t
#       }
#   stopCluster(cl)
#   cat('\n')
#   Sys.time() - timer
#   save(shap.county, file = paste0('_results/', geom, '_shapcounty_', id, '.Rdata'))
# }
