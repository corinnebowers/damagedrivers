...
## latest modification (4/30): changed damage definition, flipped y->ynew
###################################################################################################
...
#### underrepresentation ##########################################################################
# cat('correcting underrepresented geographies...\n')
...
# 
# boot <- 10
# pb <- txtProgressBar(min = 0, max = length(empty), style = 3)
...
#       } %>% apply(1, function(x) 1-mean(x))
#   yestrue <- df$ynew[df$id==i] == 'yes'
#   limit <- sum(yestrue)
#   df$ynew[df$id == i & df$y == 'no'] <- 
#     ifelse(rank(yesprob[!yestrue]) %in% 1:(5-limit), 'yes', 'no')
#   setTxtProgressBar(pb, which(empty==i))
# }
# stopCluster(cl)
# 
# ## report performance
# table(df$y, df$ynew)
# 
# ## rename outcome variable to avoid confusion
# df <- df %>% select(-y) %>% rename(y = ynew)
# rm(df.temp)
...
# vif(glm(y ~ ., data = df.pca, family = 'binomial')) %>% round(2) %>% sort(decreasing = TRUE)
# plot_contribution(df.pca, plot.dim = 7)
# cor(df.pca %>% select(-y)) %>% data.frame %>% 
#   select(med_struct_age) %>% arrange(desc(abs(med_struct_age)))
# pca.var.list <- c('med_struct_age', 'pre1980', 'pct_sfh', 'pct_ownocc')
# plot_components(df.pca, pca.var.list)
# predictive_power(df.pca, c('med_struct_age', 'pre1980'))
...
# plot_contribution(df.pca, plot.dim = 7)
# cor(df.pca %>% select(-y)) %>% data.frame %>% 
#   select(cdc_theme1) %>% arrange(desc(abs(cdc_theme1)))
# pca.var.list <- c('cdc_theme1', 'cdc_theme2', 'pct_dac', 'logpop', 'pct_mobile')
# plot_components(df.pca, pca.var.list)
# predictive_power(df.pca, c('cdc_theme1', 'cdc_theme2'))
df.pca <- df.pca %>% select(-cdc_theme1)
# pca.var.list <- c('cdc_theme2', 'pct_dac', 'logpop', 'pct_mobile')
# plot_components(df.pca, pca.var.list)
# predictive_power(df.pca, c('pct_dac', 'pct_mobile'))
...
#   mutate(pair = rep(c(1,2), nrow(.)/2)) %>% filter(pair == 1) %>% select(-pair) %>% 
#   head
# predictive_power(df.pca, c('logpop', 'pct_developed'))
