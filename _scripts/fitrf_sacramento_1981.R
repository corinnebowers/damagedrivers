...
# pca.var.list <- c('med_struct_age', 'pct_over40', 'logpop', 'loghu', 'pct_imperv', 'pct_developed')
# plot_components(df.pca, pca.var.list)
...
## 
# plot_contribution(df.pca, 7)
# pca.var.list <- c('pct_imperv', 'pct_developed', 'pct_sfh', 'pct_ownocc')
# plot_components(df.pca, pca.var.list)
# predictive_power(df.pca, c('pct_developed', 'pct_imperv'))
df.pca <- df.pca %>% select(-pct_imperv)
...
#   filter(rowname != name) %>% arrange(desc(abs(value))) %>%
#   mutate(pair = rep(c(1,2), nrow(.)/2)) %>% filter(pair == 1) %>% select(-pair) %>% head
# pca.var.list <- c('cdc_theme1', 'cdc_theme3', 'pct_white_nonhisp', 'cal_popchar')
# plot_components(df.pca, pca.var.list)
# predictive_power(df.pca, pca.var.list)
