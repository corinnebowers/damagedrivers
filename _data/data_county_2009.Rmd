...
## check 
ggplot(pop) + 
  geom_line(aes(x = year, y = pop, group = id)) + 
...
hu <- 
  map_dfr(
    ...
    .f = ~getCensus(
      ...
      regionin = 'state:06', region = 'county:*') %>% 
      mutate(year = .x)) %>% 
  transmute(county, year, hu = B25001_001E)
hu <- hu %>% 
  left_join(california %>% st_drop_geometry %>% select(COUNTYFP,NAME), by = c('county' = 'COUNTYFP')) %>% 
  left_join(polys %>% st_drop_geometry, by = c('NAME' = 'name'))
...
## check 
ggplot(hu) + 
  geom_line(aes(x = year, y = hu, group = id)) + 
  scale_y_log10() + annotation_logticks(sides = 'l', size = 0.25, color = 'grey25') + 
  labs(x = 'Year', y = 'County-Level Housing Unit Totals')
...
catalogs <- 
  foreach (j = 1:nrow(polys)) %do% {
    ...
    catalogs[[j]] %>%
      mutate(year = year(start)) %>% 
      left_join(hu %>% filter(id == j) %>% select(hu, year), by = 'year') %>% 
      select(-year) %>%
...
## check
ggplot(income %>% filter(year >= 2009)) + 
...
struct_age <- dp04 %>% 
  ...
  select(county, year, description, estimate) %>% 
  mutate(
...
med_age <- struct_age %>% 
  ...
  rbind(
    expand.grid(county = unique(.$county), year = 2009:2021) %>% 
...
pct_over40 <- struct_age %>% 
  ...
  left_join(california %>% st_drop_geometry, by = c('county' = 'COUNTYFP')) %>% 
  left_join(polys %>% st_drop_geometry, by = c('NAME' = 'name')) %>% 
  select(id, year, pct_over40)

```

```{r}
ggplot(med_age) + 
  geom_line(aes(x = year, y = med_struct_age, group = id)) + 
  labs(x = 'Year', y = 'Median Structural Age of Housing Stock (years)')

ggplot(pct_over40) + 
  geom_line(aes(x = year, y = pct_over40, group = id)) + 
...
catalogs <- 
  foreach (j = 1:nrow(polys)) %do% {
    ...
    catalogs[[j]] %>% 
      mutate(year = year(start)) %>% 
      left_join(med_age, by = c('year', 'id')) %>% 
...
housingchar <- dp04 %>% 
  ...
  left_join(polys %>% st_drop_geometry %>% select(name,id), by = c('NAME' = 'name')) %>% 
  arrange(id) %>% 
  select(year, id, pct_sfh, pct_mobile, pct_ownocc)
...
The final dataset has 5,659 individual observations recorded across 43 years and 58 counties. 
...
save(catalog.df, file = '_data/_checkpoints/county2009_catalog_1120.Rdata')
write.csv(catalog.df, file = '_data/data_county_2009.csv', row.names = FALSE)
